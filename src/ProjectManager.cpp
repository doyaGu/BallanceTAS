...
std::unique_ptr<TASProject> ProjectManager::LoadDirectoryProject(const std::string &projectPath) {
    ...
    if (!manifest_table.valid()) {
        m_Mod->GetLogger()->Warn("Could not parse manifest for directory project at: %s", projectPath.c_str());
        return nullptr;
    }
...
std::unique_ptr<TASProject> ProjectManager::LoadZipProject(const std::string &zipPath) {
    ...
    if (!ReadFileFromZip(zipPath, "manifest.lua", manifestContent)) {
        m_Mod->GetLogger()->Warn("Could not read manifest from zip project: %s", zipPath.c_str());
        return nullptr;
    }
    ...
    try {
        ...
        if (!tempFile.is_open()) {
            m_Mod->GetLogger()->Error("Could not create temporary manifest file: %s", tempManifest.c_str());
            return nullptr;
        }
        ...
        if (!manifest_table.valid()) {
            m_Mod->GetLogger()->Warn("Could not parse manifest for zip project: %s", zipPath.c_str());
            return nullptr;
        }
        ...
        m_Mod->GetLogger()->Error("Exception loading zip project %s: %s", zipPath.c_str(), e.what());
        return nullptr;
    }
}
...
std::string ProjectManager::PrepareProjectForExecution(TASProject *project) {
    ...
    if (it != m_ProjectTempDirectories.end()) {
        ...
        if (fs::exists(it->second) && ValidateProjectStructure(it->second)) {
            m_Mod->GetLogger()->Info("Using existing extracted project: %s", it->second.c_str());
            return it->second;
        } else {
    ...
    if (!ExtractZipProject(project->GetPath(), tempDirPath)) {
        m_Mod->GetLogger()->Error("Failed to extract zip project for execution: %s", project->GetPath().c_str());
    ...
    if (!ValidateProjectStructure(tempDirPath)) {
        m_Mod->GetLogger()->Error("Extracted zip project has invalid structure: %s", tempDirPath.c_str());
        RemoveDirectory(tempDirPath);
        return "";
    }
    ...
    m_Mod->GetLogger()->Info("Successfully prepared zip project for execution: %s -> %s",
...
bool ProjectManager::ExtractZipProject(const std::string &zipPath, const std::string &tempDir) {
    m_Mod->GetLogger()->Info("Extracting zip project: %s to %s", zipPath.c_str(), tempDir.c_str());

    // Create temporary directory
    try {
        if (!fs::create_directories(tempDir)) {
            if (!fs::exists(tempDir)) {
                m_Mod->GetLogger()->Error("Failed to create temp directory: %s", tempDir.c_str());
                return false;
            }
        }
    } catch (const fs::filesystem_error &e) {
        m_Mod->GetLogger()->Error("Filesystem error creating temp directory %s: %s", tempDir.c_str(), e.what());
        return false;
    }
    ...
    if (!zip) {
        m_Mod->GetLogger()->Error("Failed to open zip file: %s", zipPath.c_str());
    ...
    if (totalEntries <= 0) {
        m_Mod->GetLogger()->Error("Zip file appears to be empty or corrupted: %s", zipPath.c_str());
        zip_close(zip);
        RemoveDirectory(tempDir);
        return false;
    }

    m_Mod->GetLogger()->Info("Extracting %d entries from zip file...", static_cast<int>(totalEntries));

    for (ssize_t i = 0; i < totalEntries; ++i) {
        if (zip_entry_openbyindex(zip, i) != 0) {
            m_Mod->GetLogger()->Error("Failed to open zip entry at index %d", static_cast<int>(i));
            success = false;
            break;
        }
        ...
        try {
            ...
            m_Mod->GetLogger()->Error("Failed to create directory for %s: %s", outputPath.c_str(), e.what());
            zip_entry_close(zip);
            success = false;
            break;
        }
        ...
        if (zip_entry_isdir(zip)) {
            try {
                ...
                m_Mod->GetLogger()->Error("Failed to create directory %s: %s", outputPath.c_str(), e.what());
                success = false;
            }
            zip_entry_close(zip);
            continue;
        ...
        if (zip_entry_fread(zip, outputPath.c_str()) != 0) {
            m_Mod->GetLogger()->Error("Failed to extract file: %s", outputPath.c_str());
            success = false;
            zip_entry_close(zip);
            break;
        }
    ...
    if (!success) {
        m_Mod->GetLogger()->Error("Failed to extract zip project, cleaning up temp directory");
        RemoveDirectory(tempDir);
        return false;
    }

    m_Mod->GetLogger()->Info("Successfully extracted zip project to: %s", tempDir.c_str());
    return true;
}

bool ProjectManager::CompressProject(const std::string &projectPath, const std::string &zipPath) {
    m_Mod->GetLogger()->Info("Compressing project: %s to %s", projectPath.c_str(), zipPath.c_str());

    if (!ValidateProjectStructure(projectPath)) {
        m_Mod->GetLogger()->Error("Project structure validation failed: %s", projectPath.c_str());
        return false;
    }
    ...
    try {
        ...
        m_Mod->GetLogger()->Error("Error scanning project directory: %s", e.what());
        return false;
    }

    if (files.empty()) {
        m_Mod->GetLogger()->Error("No files found in project directory: %s", projectPath.c_str());
        return false;
    }
    ...
    if (!zip) {
        m_Mod->GetLogger()->Error("Failed to create zip file: %s", zipPath.c_str());
        return false;
    }
    ...
    for (const auto &[filePath, relativePath] : files) {
        if (zip_entry_open(zip, relativePath.c_str()) != 0) {
            m_Mod->GetLogger()->Error("Failed to open zip entry: %s", relativePath.c_str());
            success = false;
            break;
        }

        if (zip_entry_fwrite(zip, filePath.c_str()) != 0) {
            m_Mod->GetLogger()->Error("Failed to write file to zip: %s", filePath.c_str());
            success = false;
            zip_entry_close(zip);
            break;
        }
    ...
    if (success) {
        m_Mod->GetLogger()->Info("Successfully compressed project with %d files", static_cast<int>(files.size()));
    } else {
...
bool ProjectManager::ValidateZipProject(const std::string &zipPath) {
    ...
    if (!isValid) {
        m_Mod->GetLogger()->Warn("Zip project validation failed: %s (manifest: %s, main: %s)",
...
bool ProjectManager::ReadFileFromZip(const std::string &zipPath, const std::string &fileName, std::string &content) {
    ...
    if (!success) {
        m_Mod->GetLogger()->Error("Failed to read file '%s' from zip: %s", fileName.c_str(), zipPath.c_str());
    }
...
bool ProjectManager::RemoveDirectory(const std::string &dirPath) {
    try {
        ...
        m_Mod->GetLogger()->Error("Failed to remove directory '%s': %s", dirPath.c_str(), e.what());
        return false;
    }
}

void ProjectManager::CleanupTempDirectories() {
    m_Mod->GetLogger()->Info("Cleaning up temporary directories");
    ...
    for (auto it = m_ProjectTempDirectories.begin(); it != m_ProjectTempDirectories.end();) {
        ...
        if (fs::exists(tempDir)) {
            if (RemoveDirectory(tempDir)) {
                m_Mod->GetLogger()->Info("Cleaned up temp directory: %s", tempDir.c_str());
            }
        }
    ...
    try {
        ...
        m_Mod->GetLogger()->Warn("Error during temp cleanup: %s", e.what());
    }
}
...
void ProjectManager::CleanupProjectTempDirectory(TASProject *project) {
    ...
    if (it != m_ProjectTempDirectories.end()) {
        ...
        if (fs::exists(tempDir)) {
            if (RemoveDirectory(tempDir)) {
                m_Mod->GetLogger()->Info("Cleaned up temp directory for project %s: %s",
